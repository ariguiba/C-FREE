...
#         self.num_molecules = 19239  # will be set later
#         super().__init__(
#             root,
#             transform,
#             pre_transform,
...
import torch as pt
from torch_geometric.data import Data, download_url
from torchmdnet.datasets.spice import SPICE

def get_idx_split(num_molecules, train_ratio=0.8, valid_ratio=0.1, seed=123):
        molecule_ids = shuffle(range(num_molecules), random_state=seed)
        train_size = int(train_ratio * num_molecules)
        valid_size = int(valid_ratio * num_molecules)

        train_idx = pt.tensor(molecule_ids[:train_size])
        valid_idx = pt.tensor(molecule_ids[train_size : train_size + valid_size])
        test_idx = pt.tensor(molecule_ids[train_size + valid_size :])

        return train_idx, valid_idx, test_idx

def get_spice(datapath, pre_transform): #args):
    dataset = SPICE(root=datapath, version="1.1.3")
    
    # ds = SPICE(root=datapath, pre_transform=None)
    # is_subset = True
    ...
    # train_idx, val_idx, test_idx = get_idx_split(num_molecules=len(ds),
    ...
    # dataset.mmaps.y = dataset.mmaps.y.float()[:, None]
    ...
    means.append(-135.66193793642145)          # y[:, 0]
    stds.append(71.1838322055336)
