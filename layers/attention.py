...
class TfModule(torch.nn.Module):
    def __init__(
        self,
        dim_h,
        input_dim_h, # For LayerNorm before MLP proj
        num_heads,
        num_layers,
        dropout,
        new_arch=True,
        ...
        self.proj_to_tf = torch.nn.Linear(input_dim_h, dim_h)
        self.attn_layers = torch.nn.ModuleList(
            [
                AttnModule(dim_h, num_heads, dropout)
                for _ in range(num_layers)
            ]
        )

        self.proj_norm = torch.nn.LayerNorm(input_dim_h)
