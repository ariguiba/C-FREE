...
from utils.masking import mask_node_features

def get_node_encoder(dataset, hidden_dim, lap_dim=0, rwse_dim=0, tokenizer=None,):
    ...
    if dataset.lower() in ["esol", "bace", "freesolv", "lipo"]:
        ...
        return AtomEncoder(hidden_dim, tokenizer)
...
class AtomEncoder(torch.nn.Module): 
    """
    Converts atoms into tokens and returns their embeddings 
    A token is a unique configuration of atom features 
    """
    def __init__(self, hidden, tokenizer): 
        super(AtomEncoder, self).__init__()
        self.tokenizer = tokenizer

        self.embedding = torch.nn.Embedding(
            num_embeddings=self.tokenizer.vocab_size, embedding_dim=hidden)
    ...
    def forward(self, data):
        x = data.x
        #mask = mask_node_features(data, 0.0) # Mask ratio
        tokens = torch.tensor(
            self.tokenizer.encode(x.tolist(), None), dtype=torch.int64).to(x.device)
